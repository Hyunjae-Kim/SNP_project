import numpy as np
import torch

def sort012_num(snp_data):
    len_data = len(snp_data)
    num0 = np.sum(snp_data==0)
    num1 = np.sum(snp_data==1)
    num2 = np.sum(snp_data==2)
    num_mut = num1+num2
    return num0, num1, num2, num_mut
    
    
def load_data(gene_data_name, gene_name, tissue_num, proc=True):
    np.random.seed(37)
    
    snp_name = '../../%s/%s/%s__snps.npy'%(gene_data_name, gene_name, gene_name)
    gx_name = '../../%s/%s/%s__gene_expression_levels.npy'%(gene_data_name, gene_name, gene_name)
#     sort_idx = np.load('../main_code/sorted_sample_idx.npy')-1
#     print(np.shape(sort_idx))
    
    a = np.float32(np.load(snp_name).transpose())#[sort_idx]
    b = np.float32(np.load(gx_name).transpose())#[sort_idx]
    
    no_nan_arg = np.nan_to_num(np.abs(b[:,tissue_num-1])+1)>0
    no_nan_idx = np.where(no_nan_arg)[0]
    snp_data = a[no_nan_idx, :]
    gx_data = np.nan_to_num(b[no_nan_idx, :44])
    mask_data = np.float32(np.nan_to_num(np.abs(b[no_nan_idx, :44])+1)>0)

    if proc:
        tiss_mean = np.mean(gx_data, axis=0)
        gx_data = (gx_data - tiss_mean)/tiss_mean
        s = np.arange(len(gx_data))
        np.random.shuffle(s)
        
        snp_data = snp_data[s]
        gx_data = gx_data[s]
        mask_data = mask_data[s]
        
    return snp_data, gx_data, mask_data
    
    
def k_fold_data(snp_data, gx_data, mask_data, div_num, k_num):
    if div_num>10:
        print("Error : div_num should be < 10")
        raise ValueError
        
    elif k_num<=0 or k_num>div_num:
        print("Error : k_num should > 1 and <= div_num")
        raise ValueError
    
    data_len = len(snp_data)
    div_len = int(data_len/div_num)
    
    if div_num==k_num:
        start_te_idx = div_len*(k_num-1)
        end_te_idx = data_len
        
        start_vad_idx = 0
        end_vad_idx = div_len
        
        snp_train = snp_data[end_vad_idx:start_te_idx]
        gx_train = gx_data[end_vad_idx:start_te_idx]
        mask_train = mask_data[end_vad_idx:start_te_idx]
        
    else:
        start_te_idx = div_len*(k_num-1)
        end_te_idx = div_len*k_num
        
        start_vad_idx = div_len*k_num
        if (div_num-1)==k_num:
            end_vad_idx = data_len
        else:
            end_vad_idx = div_len*(k_num+1)
        
        snp_train = np.vstack((snp_data[:start_te_idx,:], snp_data[end_vad_idx:,:]))
        gx_train = np.vstack((gx_data[:start_te_idx,:], gx_data[end_vad_idx:,:]))
        mask_train = np.vstack((mask_data[:start_te_idx,:], mask_data[end_vad_idx:,:]))
        
    snp_valid = snp_data[start_vad_idx:end_vad_idx]
    gx_valid = gx_data[start_vad_idx:end_vad_idx]
    mask_valid = mask_data[start_vad_idx:end_vad_idx]
    
    snp_test = snp_data[start_te_idx:end_te_idx]
    gx_test = gx_data[start_te_idx:end_te_idx]
    mask_test = mask_data[start_te_idx:end_te_idx]
    
    return snp_train, snp_valid, snp_test, gx_train, gx_valid, gx_test, mask_train, mask_valid, mask_test
    
    
def sample_dist(input_data):
    data_sum = torch.sum(input_data**2, dim=1, keepdim=True)
    data_mat = -2*torch.matmul(input_data, input_data.transpose(1,0)) + data_sum
    data_dis = data_mat.transpose(1,0) + data_sum
    return data_dis
    
    