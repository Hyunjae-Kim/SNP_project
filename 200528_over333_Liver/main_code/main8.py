...
s_size_list = [4, 8]
lasso_list = [0.3]
lasso_alpha = lasso_list[0]
n_channel = ch_list[0]
k_size = k_size_list[model_num-1]
s_size = s_size_list[model_num-1]
...
print('Option : Lasso alpha / # of channel : %.4f / %d'\
      %(lasso_alpha, n_channel))
...
snp = np.eye(3)[snp.astype(int)][:, np.newaxis, :,:].transpose(0,1,3,2)
...
model = nn.DataParallel(Model(snp_len=snp_len, n_channel=n_channel, k_size=k_size, s_size=s_size)).to(device)
...
for i in range(2001):
    ...
    for mb in range(mb_div):
        ...
        tr_loss = loss_func(output, dgx_train, l1_tr, lasso_alpha)
        tr_loss.backward()
        optimizer.step()
