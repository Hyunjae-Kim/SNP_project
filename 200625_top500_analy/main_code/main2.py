...
print('Option - lasso coeff : %.5f / base_nodes : %d'%(lasso_alpha, base_node))
...
for i in range(2001):
    ...
    if i%100==0:
        ...
        if te_loss_buff>=te_loss2.cpu().item():
            ...
            w1 = model.module.fc1[0].weight.cpu().detach().numpy()
            w2 = model.module.fc2.weight.cpu().detach().numpy()
            b1 = model.module.fc1[0].bias.cpu().detach().numpy()
            b2 = model.module.fc2.bias.cpu().detach().numpy()
            
            np.save('../ckpt/trial%d/gene%d/model%d/%dtissue_k%d_w1.npy'\
                    %(trial_num, gene_num, model_num,tissue_num, k_num), w1)
            np.save('../ckpt/trial%d/gene%d/model%d/%dtissue_k%d_w2.npy'\
                    %(trial_num, gene_num, model_num,tissue_num, k_num), w2)
            np.save('../ckpt/trial%d/gene%d/model%d/%dtissue_k%d_b1.npy'\
                    %(trial_num, gene_num, model_num,tissue_num, k_num), b1)
            np.save('../ckpt/trial%d/gene%d/model%d/%dtissue_k%d_b2.npy'\
                    %(trial_num, gene_num, model_num,tissue_num, k_num), b2)
...
np.save('../ckpt/trial%d/gene%d/model%d/%dtissue_k%d_w1_last.npy'\
        %(trial_num, gene_num, model_num,tissue_num, k_num), w1)
np.save('../ckpt/trial%d/gene%d/model%d/%dtissue_k%d_w2_last.npy'\
        %(trial_num, gene_num, model_num,tissue_num, k_num), w2)
np.save('../ckpt/trial%d/gene%d/model%d/%dtissue_k%d_b1_last.npy'\
        %(trial_num, gene_num, model_num,tissue_num, k_num), b1)
np.save('../ckpt/trial%d/gene%d/model%d/%dtissue_k%d_b2_last.npy'\
        %(trial_num, gene_num, model_num,tissue_num, k_num), b2)
