...
lasso_list = [0.005, 0.05, 0.1, 0.3, 0.5]
...
print('Option - lasso coeff : %.5f'%lasso_alpha)
...
model = nn.DataParallel(Model(snp_len=snp_train.size()[-1])).to(device)
...
for i in range(5001):
    ...
    if i%100==0:
        ...
        if te_loss_buff>=te_loss2.cpu().item():
            ...
            w1 = model.module.fc1.weight.cpu().detach().numpy()
            b1 = model.module.fc1.bias.cpu().detach().numpy()
