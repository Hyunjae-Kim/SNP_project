...
print('\n\n[Gene %d] Model %d ( All tissue ) - %d/5 fold data'%(gene_num, model_num, k_num))
...
snp, gx, mask = module.load_data(gene_data_name, gene_name, proc=True)
snp_train, snp_valid, snp_test, gx_train, gx_valid, gx_test, mask_train, mask_valid, mask_test = \
                                module.k_fold_data(snp, gx, mask, 5, k_num)
...
def loss_func(output, target, tiss_num, weight_norm, lasso_coeff):
    l = torch.mean((output[:,tiss_num]-target[:,tiss_num])**2)
...
for i in range(1001):
    ...
    for mb in range(mb_div):
        ...
        tr_loss = loss_func(output*dmask_train, dgx_train, tissue_num, l1_tr, lasso_alpha)
        tr_loss.backward()
        optimizer.step()
    ...
    if i%100==0:
        ...
        if vad_loss_buff>=vad_loss.cpu().item():
            ...
            np.save('../npy/trial%d/gene%d/model%d/%stissue_trSNP2_k%d.npy'%(trial_num, gene_num, model_num, \
                                                    'all', k_num), output.cpu().detach().numpy())
            np.save('../npy/trial%d/gene%d/model%d/%stissue_teSNP2_k%d.npy'%(trial_num, gene_num, model_num, \
                                                    'all', k_num), out_test.cpu().detach().numpy())
                
np.save('../npy/trial%d/gene%d/model%d/%stissue_trSNP2_k%d_last.npy'%(trial_num, gene_num, model_num, \
                                                    'all', k_num), output.cpu().detach().numpy())
np.save('../npy/trial%d/gene%d/model%d/%stissue_teSNP2_k%d_last.npy'%(trial_num, gene_num, model_num, \
                                                    'all', k_num), out_test.cpu().detach().numpy())
np.save('../npy/trial%d/gene%d/model%d/%stissue_trGX_k%d.npy'%(trial_num, gene_num, model_num, 'all', \
                                                         k_num), gx_train.cpu().detach().numpy())
np.save('../npy/trial%d/gene%d/model%d/%stissue_teGX_k%d.npy'%(trial_num, gene_num, model_num, 'all', \
...
plt.ylabel('Loss')
plt.legend()
plt.title('Model %d , Tissue %s (Min loss %d th iter) - train : %.4f  /  test : %.4f'%(model_num, \
                                                         'all', min_iter, tr_loss_buff, te_loss_buff))
plt.savefig('../img/trial%d/gene%d/loss_plot/model%d_tissue_%s_k%d.png'%(trial_num, gene_num, model_num, \
                                                                     'all', k_num))
plt.clf()
