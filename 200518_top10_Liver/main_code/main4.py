...
drop_prob_list = [0.5, 0.9, 0.99]
lasso_list = [0.0003, 0.003, 0.03, 0.1, 0.3]

if model_num<4:
    drop_prob = drop_prob_list[model_num-1]
    lasso_alpha = 0
if model_num>=4:
    drop_prob=0.5
    lasso_alpha = lasso_list[model_num-4]
base_node = 4096
...
print('Option : Lasso alpha / # of base nodes / Drop prob : %.4f / %d  / %.2f'\
      %(lasso_alpha, base_node, drop_prob))
...
model = nn.DataParallel(Model(snp_len=snp_train.size()[-1], base_node=base_node, 
