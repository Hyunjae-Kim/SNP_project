...
k_len_list = [8, 16, 32, 64, 128]
k_len = k_len_list[model_num-1]
lasso_alpha = 0.00005
...
print('Option - kernel length : %d / lasso coeff : %.5f '%(k_len, lasso_alpha))
...
input_ch_num = int(snp_len/k_len)
snp = snp[:, :input_ch_num*k_len]
snp = np.eye(3)[snp.astype(int)].reshape(-1, input_ch_num, k_len, 3).transpose(0,1,3,2)
...
learning_rate = 0.005
model = nn.DataParallel(Model(k_len=k_len, input_ch_num=input_ch_num)).to(device)
